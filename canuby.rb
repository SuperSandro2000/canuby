...
namespace :thirdparty do
  ...
  task :init do
    mkdir_p base_dir unless Dir.exists?(base_dir)
  end

  desc "Prepare googletest"
  task googletest: 'googletest:staged'

  namespace :googletest do |namespace|
    def project_dir
    ...
    def clean_stage
      stage_outputs.each {|f| rm f if File.exists?(f)}
    end
    ...
    task cloned: :init do
      do_clone unless File.exists?(project_dir)
    end

    task built: :cloned do
      do_build unless build_outputs.all? {|f| File.exists?(f)}
    end

    task staged: :built do
      do_stage unless stage_outputs.all? {|f| File.exists?(f)}
    end
